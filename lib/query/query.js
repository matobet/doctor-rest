...
class Query {
  ...
  static one(name, id, query) {
    let q = new Query(name);
      ...
      object: () => {
        ...
        if ('ref' in selector) {
          if (!(selector.ref in q.refs)) {
            ...
            q.refs[selector.ref] = new Query(selector.ref);
          }
          ...
          if (!(selector.many_ref in q.arrayRefs)) {
            ...
            q.arrayRefs[selector.many_ref] = new Query(selector.many_ref);
          }
  ...
  loadEntityCollectionData(parentId, filter) {
    ...
    return co(function *() {
      ...
      yield entities.filter(e => e._links).map(q.getEntityLoadPromise.bind(q));
      return entities;
    });
  }
    ...
    selects.forEach(select => {
      ...
      if ('ref' in select) {
        ...
        if (subSelect) {
          util.switchStringArrayObject(subSelect, {
            string: () => {
              obj[`${prefix}@[${select.many_ref}].${subSelect}`] = refData.map(e => e[subSelect]);
            },
            array: () => {
