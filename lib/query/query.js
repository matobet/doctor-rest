...
class Query {
  ...
  static one(name, id, query, permission) {
    let q = new OneQuery(name, permission);
    if (query) {
      q.parseQuery(query);
    }
  ...
  static many(name, query, permission) {
    let q = new ManyQuery(name, permission);
    if (query) {
      q.parseQuery(query);
    }
      ...
      object: () => {
        ...
        if ('ref' in selector) {
          if (!(selector.ref in q.refs)) {
            ...
            q.refs[selector.ref] = new OneQuery(selector.ref, q.permission);
          }
          ...
          if (!(selector.many_ref in q.arrayRefs)) {
            ...
            q.arrayRefs[selector.many_ref] = new ManyQuery(selector.many_ref, q.permission);
          }
  ...
  getRequiredFieldsQuery() {
    if (this.fullData || (this.fields.size === 0 && util.isEmptyObject(this.refs) && util.isEmptyObject(this.arrayRefs))) {
      return _.cloneDeep(db.NO_MONGO_ID);
    }
...
class ManyQuery extends Query {
  ...
  getFilter(filter) {
    ...
    if (this.permission.restricts(this.name)) {
      let idWhiteList = this.permission.getIdWhiteListFor(this.name);
      if (!('id' in filter)) {
        filter.id = { $in: idWhiteList };
      } else {
        // here we assume that 'id' field is filtered always using the $in operator
        // TODO: this will break if user supplies filter on 'id' with direct value
        // TODO: consider using intersection of requested and permitted entities, instead of 403
