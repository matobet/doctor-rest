...
class Query {
  ...
  getRequiredFieldsQuery () {
    let query
    if (this.fullData || (_.isEmpty(this.fields) && _.isEmpty(this.refs) && _.isEmpty(this.arrayRefs))) {
  ...
  applyAcl (filter) {
    if (this.user && this.user !== '*') {
      // if document doesn't contain '_acl' field, it is considered public
      filter.$or = [{_acl: this.user}, {_acl: {$exists: false}}]
    }
  ...
  getRequiredFields () {
    const directFields = Array.from(this.fields)
  ...
  getEntityLoadPromise (entity) {
    let promises = {}
    promises.refs = _.mapValues(this.refs, (query, ref) => {
      return Query._isValidLink(entity[ref]) && query.loadEntityData(entity[ref].toString())
    })
    promises.arrayRefs = _.mapValues(this.arrayRefs, (query, ref) => {
      if (_.isArray(entity[ref])) {
        let ids = Array.from(entity[ref]).map((id) => id.toString())
        return query.loadEntityCollectionData(entity.id, {id: {$in: ids}})
      } else {
        db.ensureIndex(ref, this.name)
  ...
  static pickRefFields (refData, select) {
    return _.includes(select, '*') ? refData : _.pick(refData, select)
  }
